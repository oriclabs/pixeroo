  ...
  wasmLoading = (async () => {
    try {
      ...
      const wasmUrl = typeof chrome !== 'undefined' && chrome.runtime?.getURL
        ? chrome.runtime.getURL('wasm/pixeroo_wasm_bg.wasm')
        : '/shared/wasm/pkg/pixeroo_wasm_bg.wasm';

      const { default: init, ...exports } = await import('./pkg/pixeroo_wasm.js');
      await init(wasmUrl);

      wasmInstance = exports;
      console.log(`[Pixeroo] WASM engine v${exports.version()} ready`);
      return wasmInstance;
    } catch (e) {
      console.error('[Pixeroo] WASM load failed:', e);
      wasmLoading = null;
      throw e;
    }
  })();
