  ...
  try {
    ...
    try {
      ...
      if (isUrlRestricted || isBlocked) {
        ...
        // Normal page loaded before Gazo — show reload button
        showPageColorsTabs();
        gallery.style.display = 'none';
        ...
        extMsg.innerHTML = `
          ...
          <p style="color:var(--slate-400);line-height:1.5;">The page was loaded before Gazo. Reload to enable page image scanning and color picking.</p>
...
function initDownload() {
  $('btn-dl-selected').addEventListener('click', () => {
    ...
    if (images.length > 0) downloadImagesAsZip(images, 'gazo-selected.zip');
  });

  $('btn-dl-all').addEventListener('click', () => {
    const images = getFilteredImages();
    if (images.length > 0) downloadImagesAsZip(images, 'gazo-images.zip');
  });
}
  ...
  try {
    ...
    if (response?.error) {
      ...
      const msg = isPermission
        ? 'Click the Gazo icon in the toolbar first, then try again. This grants permission to capture the current tab.'
...
function downloadScreenshot() {
  ...
  chrome.runtime.sendMessage({
    ...
    filename: `gazo/screenshot-${timestamp}.png`,
    saveAs: true
  });
}
...
async function captureRegionScreenshot() {
  ...
  try {
    ...
    const msg = isPermission
      ? 'Click the Gazo icon in the toolbar first, then try again. This grants permission to access the current page.'
...
$('gallery')?.addEventListener('contextmenu', (e) => {
  ...
  const items = [
    ...
    },
    {
      ...
      action: () => {
        chrome.runtime.sendMessage({
          action: 'download',
          url: img.src,
          filename: 'gazo/' + (img.filename || extractFilename(img.src) || 'image'),
          saveAs: true
        });
      }
    },
    ...
    'sep',
    {
      ...
      icon: _ctxIcons.edit,
      action: async () => {
        await chrome.storage.local.set({ 'gazo-lib-transfer': { tool: 'edit', images: [img.src] } });
