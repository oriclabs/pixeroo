...
async function initLibrary() {
  ...
  $('btn-lib-export-selected')?.addEventListener('click', () => {
    if (libSelectedIds.size) exportLibraryAsZip([...libSelectedIds], 'gazo-selected.zip');
  });
  $('btn-lib-export-all')?.addEventListener('click', async () => {
    ...
    if (ids.length) exportLibraryAsZip(ids, 'gazo-library.zip');
  });
...
async function sendLibToTool(tool) {
  ...
  await chrome.storage.local.set({ 'gazo-lib-transfer': { tool, images } });
  ...
  const items = [
    {
      ...
      action: async () => {
        ...
        await chrome.storage.local.set({ 'gazo-lib-transfer': { tool: 'edit', images: [item.dataUrl] } });
    ...
    },
    {
      ...
      action: async () => {
        const item = await PixLibrary.get(id);
        if (!item?.dataUrl) return;
        await chrome.storage.local.set({ 'gazo-lib-transfer': { tool: 'collage', images: [item.dataUrl] } });
    ...
    },
    {
      ...
      action: async () => {
        const item = await PixLibrary.get(id);
        if (!item?.dataUrl) return;
        await chrome.storage.local.set({ 'gazo-lib-transfer': { tool: 'batch', images: [item.dataUrl] } });
    ...
    'sep',
    {
      ...
      action: async () => {
        ...
        chrome.runtime.sendMessage({
          action: 'download',
          url: item.dataUrl,
          filename: 'gazo/' + (item.name || `library-image.${ext}`),
          saveAs: true
        });
      }
    },
