...
function initDraw() {
  ...
  // ── Font ──
  $('draw-font')?.addEventListener('change', (e) => {
    if (drawObjLayer) {
      drawObjLayer.fontFamily = e.target.value;
      if (drawObjLayer.selected?.type === 'text') { drawObjLayer.selected.fontFamily = e.target.value; drawObjLayer.render(); }
    }
  });
  $('draw-fontsize')?.addEventListener('input', (e) => {
    if (drawObjLayer) {
      drawObjLayer.fontSize = +e.target.value || 24;
      if (drawObjLayer.selected?.type === 'text') { drawObjLayer.selected.fontSize = +e.target.value || 24; drawObjLayer.render(); }
    }
  });

  // ── Bold / Italic / Underline ──
  $('draw-bold')?.addEventListener('click', (e) => {
    const bold = e.currentTarget.classList.toggle('active') ? 'bold' : 'normal';
    if (drawObjLayer) { drawObjLayer.fontWeight = bold; if (drawObjLayer.selected?.type === 'text') { drawObjLayer.selected.fontWeight = bold; drawObjLayer.render(); } }
  });
  $('draw-italic')?.addEventListener('click', (e) => {
    const italic = e.currentTarget.classList.toggle('active') ? 'italic' : 'normal';
    if (drawObjLayer) { drawObjLayer.fontStyle = italic; if (drawObjLayer.selected?.type === 'text') { drawObjLayer.selected.fontStyle = italic; drawObjLayer.render(); } }
  });
  $('draw-underline')?.addEventListener('click', (e) => {
    const underline = e.currentTarget.classList.toggle('active');
    if (drawObjLayer?.selected?.type === 'text') { drawObjLayer.selected.underline = underline; drawObjLayer.render(); }
  });
