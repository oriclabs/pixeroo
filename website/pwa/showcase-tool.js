...
function initShowcase() {
  ...
  // ── ExportPill ─────────────────────────────────────────
  const _scExportPill = new ExportPill($('sc-export-pill'), {
    formats: ['png', 'jpeg', 'webp'],
    showLibrary: true,
    onExport(state) {
      if (!canvas.width || !scImg) return;
      canvas.toBlob(blob => {
        Platform.download(URL.createObjectURL(blob), `gazo/showcase.${state.ext}`, true);
      }, state.mime, 0.92);
    },
    onLibrary() {
      if (!canvas.width || !scImg) return;
      ...
      if (typeof PixLibrary !== 'undefined')
        PixLibrary.add({ dataUrl, source: 'showcase', name: 'showcase', width: canvas.width, height: canvas.height, type: 'image', size: dataUrl.length });
    },
    onCopy: async () => {
      if (!canvas.width || !scImg) return;
