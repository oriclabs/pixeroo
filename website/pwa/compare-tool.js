...
function initCompare() {
  ...
  $('btn-compare-diff').addEventListener('click',()=>{
    ...
    const diffPct = ((dc/(w*h))*100).toFixed(1);
    // Compute SSIM if available
    let ssimStr = '';
    if (typeof computeSSIM === 'function') {
      const ssim = computeSSIM(cA, cB);
      ssimStr = ` | SSIM: ${ssim.toFixed(4)}`;
    }
    $('compare-info-b').textContent=`Diff: ${diffPct}% (${dc} px)${ssimStr}`;
  });
