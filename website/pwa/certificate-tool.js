...
function initCertificate() {
  ...
  // ── ExportPill ─────────────────────────────────────────
  const _certExportPill = new ExportPill($('cert-export-pill'), {
    formats: ['png', 'jpeg', 'webp'],
    showLibrary: true,
    onExport(state) {
      if (!canvas.width) return;
      canvas.toBlob(blob => {
        Platform.download(URL.createObjectURL(blob), `gazo/certificate.${state.ext}`, true);
      }, state.mime, 0.92);
    },
    onLibrary() {
      if (!canvas.width) return;
      ...
      if (typeof PixLibrary !== 'undefined') {
        PixLibrary.add({ dataUrl, source: 'certificate', name: 'certificate', width: canvas.width, height: canvas.height, type: 'image', size: dataUrl.length });
      }
    },
    onCopy: async () => {
      if (!canvas.width) return;
