...
function initBatch() {
  ...
  // FontPill for batch watermark
  if ($('batch-wm-font-pill')) window._batchWmFontPill = new FontPill($('batch-wm-font-pill'), { defaultFamily: 'Arial, sans-serif', defaultSize: 0, autoSize: true, maxSize: 200 });
  ...
  function applyPositionedWatermark(c, ctx, text, opts) {
    ...
    const fontFamily = window._batchWmFontPill?.state.family || 'Inter, system-ui, sans-serif';
    const customSize = window._batchWmFontPill?.state.size || 0;
    const fontSize = customSize > 0 ? customSize : Math.round(Math.min(c.width, c.height) * 0.05);
    const bold = window._batchWmFontPill?.state.bold ? 'bold' : 'normal';
    const italic = window._batchWmFontPill?.state.italic ? 'italic' : 'normal';
  ...
  function getBatchSettings() {
    return {
      ...
      wmFont: window._batchWmFontPill?.state.family || 'Inter, system-ui, sans-serif',
