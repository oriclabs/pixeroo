...
function initCallout() {
  ...
  let coImg = null;
  // FontPill for callout
  if ($('co-font-pill')) window._coFontPill = new FontPill($('co-font-pill'), { defaultFamily: 'Inter, system-ui, sans-serif', defaultSize: 16, minSize: 8, maxSize: 72 });
  let coObjLayer = null;
  ...
  $$('.callout-tpl').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      coObjLayer.addCallout(canvas.width / 2 - 100, canvas.height / 2 - 40, 200, 80, {
        ...tpl,
        fontSize: window._coFontPill?.state.size || 16,
      });
    });
  });
  ...
  const _coExportPill = new ExportPill($('co-export-pill'), {
    formats: ['png', 'jpeg', 'webp'],
    showLibrary: true,
    onExport(state) {
      if (!canvas.width || !coObjLayer) return;
      const out = _coFlattenCanvas();
      out.toBlob(blob => {
        Platform.download(URL.createObjectURL(blob), `gazo/callout-annotated.${state.ext}`, true);
      }, state.mime, 0.92);
    },
    onLibrary() {
      if (!canvas.width || !coObjLayer) return;
