...
function initRecentFiles() {
  ...
  clearBtn?.addEventListener('mouseleave', () => { clearBtn.style.color = 'var(--slate-500)'; });
  clearBtn?.addEventListener('click', async () => {
    try {
      await chrome.storage.local.set({ recentFiles: [] });
      $('home-recent').style.display = 'none';
      $('recent-row').innerHTML = '';
    } catch {}
  });
}
...
async function initFontManager() {
  ...
  // Init font manager — FontPills register their own selects on-demand
  await PixFontManager.init();
  // Register any remaining raw font selects (placeholder tool in hidden section)
  const rawSelects = [
    { id: 'ph-font', defaultValue: 'Inter, system-ui, sans-serif' },
  ];
  for (const { id, defaultValue } of rawSelects) {
    const el = $(id);
...
function openMode(mode) {
  ...
  // Generate tool: show size guide when panel is visible
  if (mode === 'generate' && window._updateGenSizeGuide) {
    setTimeout(window._updateGenSizeGuide, 50);
  }
}
