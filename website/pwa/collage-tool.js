...
function initCollage() {
  ...
  if ($('coll-font-pill')) window._collFontPill = new FontPill($('coll-font-pill'), { defaultFamily: 'Inter, system-ui, sans-serif', defaultSize: 36 });

  // ── ExportPill ─────────────────────────────────────────
  const _collageExportPill = new ExportPill($('collage-export-pill'), {
    ...
    onExport(state) {
      ...
      const q = state.format === 'png' ? undefined : 0.92;
      ...
      if (trim) {
        ...
        trimCanvas.toBlob(blob => {
          Platform.download(URL.createObjectURL(blob), `gazo/collage.${state.ext}`, true);
  ...
  function syncUI() {
    ...
    if (selected.type === 'text') {
      ...
      if (window._collFontPill) window._collFontPill.state = { family: selected.fontFamily || 'Inter, system-ui, sans-serif', size: selected.fontSize || 36 };
    }
  ...
  const propMap = [
    ...
    // coll-text-size and coll-text-font now handled by FontPill
  ];
  ...
  $('btn-collage-save')?.addEventListener('click', () => {
    ...
    Platform.download(url, 'gazo/collage-project.gazo', true);
