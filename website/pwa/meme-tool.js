...
function initMeme() {
  ...
  // FontPill for meme
  const memePillContainer = $('meme-font-pill');
  if (memePillContainer) {
    window._memeFontPill = new FontPill(memePillContainer, {
      defaultFamily: 'Impact, sans-serif', defaultSize: 0, autoSize: true,
      maxSize: 120,
    });
  }
  ...
  function drawMemeText(ctx, text, x, y, maxWidth, fontSize, color, strokeColor) {
    if (!text) return;
    ctx.font = `bold ${fontSize}px ${window._memeFontPill?.state.family || 'Impact, sans-serif'}`;
  ...
  function render() {
    ...
    const fontSize = window._memeFontPill?.state.size || 0;
    ...
    if (bottomText) {
      ...
      ctx.font = `bold ${fs}px ${window._memeFontPill?.state.family || 'Impact, sans-serif'}`;
  ...
  // Font size now handled by FontPill
  $('meme-padding')?.addEventListener('input', () => { $('meme-padding-val').textContent = $('meme-padding').value + '%'; });

  // ── ExportPill ─────────────────────────────────────────
  const _memeExportPill = new ExportPill($('meme-export-pill'), {
    formats: ['png', 'jpeg', 'webp'],
    showLibrary: true,
    onExport(state) {
      if (!canvas.width) return;
      canvas.toBlob(blob => {
        Platform.download(URL.createObjectURL(blob), `gazo/meme.${state.ext}`, true);
      }, state.mime, 0.92);
    },
    onLibrary() {
      if (!canvas.width) return;
      ...
      if (typeof PixLibrary !== 'undefined') {
        PixLibrary.add({ dataUrl, source: 'meme', name: 'meme', width: canvas.width, height: canvas.height, type: 'image', size: dataUrl.length });
      }
    },
    onCopy: async () => {
      if (!canvas.width) return;
