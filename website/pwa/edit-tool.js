...
function initEdit() {
  ...
  // Click-to-browse only on the file drop area, not the library button
  const editDropFile = $('edit-drop-file');
  if (editDropFile) {
    editDropFile.addEventListener('click', () => $('edit-file').click());
    editDropFile.addEventListener('mouseenter', () => { editDropFile.style.borderColor = 'var(--saffron-400)'; });
    editDropFile.addEventListener('mouseleave', () => { editDropFile.style.borderColor = 'var(--slate-700)'; });
  }
  ...
  objLayer.onSelect = (obj) => {
    ...
    // Font (text/callout) — sync FontPill
    if (obj.type === 'text' || obj.type === 'callout') {
      if (window._annFontPill) {
        window._annFontPill.state = {
          family: obj.fontFamily || 'Inter, system-ui, sans-serif',
  ...
  // Font/size — handled by FontPill (see below)
  ...
  // FontPill for annotations (replaces ann-font, ann-fontsize, ann-bold/italic/underline)
  const annPillContainer = $('ann-font-pill');
  if (annPillContainer) {
    window._annFontPill = new FontPill(annPillContainer, {
      defaultFamily: 'Inter, system-ui, sans-serif',
      defaultSize: 24,
      showUnderline: true,
      onChange(s) {
        objLayer.fontFamily = s.family;
        objLayer.fontSize = s.size;
        const sel = objLayer.selected;
        if (sel?.type === 'text' || sel?.type === 'callout') {
          sel.fontFamily = s.family;
          sel.fontSize = s.size;
