...
function initInfo() {
  setupDropzone($('info-drop'), $('info-file'), async (file) => {
    ...
    if (img) {
      ...
      $('btn-info-copy-css').disabled = false;
  ...
  // Copy CSS dimensions
  $('btn-info-copy-css')?.addEventListener('click', () => {
    const imgEl = $('info-img');
    if (!imgEl?.naturalWidth) return;
    const w = imgEl.naturalWidth, h = imgEl.naturalHeight;
    const g = gcd(w, h);
    const css = `width: ${w}px;\nheight: ${h}px;\naspect-ratio: ${w/g} / ${h/g};`;
    navigator.clipboard.writeText(css);
    showToast('CSS dimensions copied', 'success');
  });

  // Paste data URI → render
  $('btn-info-paste-b64')?.addEventListener('click', async () => {
    try {
      const text = await navigator.clipboard.readText();
      if (!text || !text.startsWith('data:image/')) {
        showToast('Clipboard does not contain a data URI', 'warn'); return;
      }
      const img = new Image();
      img.onload = () => {
        $('info-drop').style.display = 'none';
        ...
        const grid = $('info-details-grid');
        if (grid) { grid.style.display = 'grid'; grid.style.gridTemplateColumns = '1fr 1fr'; }
        const mime = text.match(/data:(image\/[^;]+)/)?.[1] || 'image/png';
        const b64 = text.split(',')[1] || '';
        const sizeBytes = Math.round(b64.length * 3 / 4);
        $('info-file-details').innerHTML = [
          ['Source', 'Data URI (pasted)'], ['Type', mime], ['Size', formatBytes(sizeBytes)],
          ['Dimensions', `${img.naturalWidth} x ${img.naturalHeight}`],
          ['Ratio', `${img.naturalWidth/gcd(img.naturalWidth,img.naturalHeight)}:${img.naturalHeight/gcd(img.naturalWidth,img.naturalHeight)}`],
        ].map(([l,v]) => `<div class="info-row"><span class="info-label">${l}</span><span class="info-value">${esc(v)}</span></div>`).join('');
        $('info-exif').innerHTML = '<span style="color:var(--slate-500);">N/A for data URI</span>';
        $('info-structure').innerHTML = '<span style="color:var(--slate-500);">N/A for data URI</span>';
        $('info-dpi').innerHTML = '<span style="color:var(--slate-500);">N/A for data URI</span>';
        $('info-hash').innerHTML = '<span style="color:var(--slate-500);">N/A for data URI</span>';
        $('btn-info-copy-css').disabled = false;
        showToast('Data URI rendered', 'success');
      };
      img.onerror = () => showToast('Invalid data URI', 'warn');
      img.src = text;
    } catch { showToast('Could not read clipboard', 'warn'); }
  });
  ...
  $('btn-info-reset')?.addEventListener('click', () => {
    ...
    $('btn-info-copy-css').disabled = true;
    $('info-file').value = '';
  });
}
